...
Chrome Web Store 发布需要注册开发者账号，并且目前有一次性注册费。如果暂时不想支付注册费，先通过 GitHub 和本地安装方式给早期用户测试也完全可行。

准备发布时：

1. 注册 Chrome Developer Dashboard。
2. 上传 `dist/easycurl-extension-0.1.0.zip`。
3. 填写商店介绍、截图、隐私字段和发布范围。
4. 提交审核。
