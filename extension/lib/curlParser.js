...
export function parseCurl(input) {
  ...
  while (index < tokens.length) {
    ...
    if (COOKIE_FLAGS.has(flag)) {
      const value = readFlagValue(tokens, index, inlineValue);
      addCookieHeader(request, value.value);
      index = value.nextIndex;
      continue;
    }
...
function addCookieHeader(request, cookieValue) {
  const value = String(cookieValue || "").trim();
  if (!value) return;

  const cookieHeader = request.headers.find((header) => header.key.toLowerCase() === "cookie");
  if (cookieHeader) {
    cookieHeader.value = [cookieHeader.value, value].filter(Boolean).join("; ");
    return;
  }
